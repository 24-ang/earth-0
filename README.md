...
**A deterministic engine + multi-agent LLM runtime for interactive fiction.**
Each NPC is an independent Agent with its own memory, knowledge, and body.
...
一个跑在 [pi-coding-agent](https://github.com/mariozechner/pi-coding-agent) 框架上的**交互式叙事运行时**。它不是一个 prompt 卡或角色卡——它有一整套确定性引擎（时间、战斗、经济、物理空间、日程、天气、NPC AI、剧情时间线、社交网络），加上一个 LLM 驱动的游戏主持人（GM）。
