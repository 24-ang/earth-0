...
function buildRenderContract(wordBudget: string, interactionMode: string): string {
  return [
    ...
    "- 对话用中文引号「」或日文引号『』",
    `- 字数: ${wordBudget}字`,
    `- 人称: ${interactionMode === "novel" ? "第三人称「他」（镜头钉在主角身边，第三人称限知视角）" : "按 Voice 层规则"}`,

    "### 输出格式",
    ...(interactionMode === "novel"
      ? ["纯文本叙事（小说模式）：",
         "---",
         "> [继续阅读] 接受当前叙事，故事自然推进到下一个时刻或场景。",
         "> [介入对话] 玩家主动打破叙事流，对场景中的角色说话或采取行动。",
         "> [自由输入] 输入任意行动文本。"]
      : ["纯文本叙事 + 4个扮演选项：",
         "---",
         "> ① [普通]: 「...」或 *行动...*",
         ...
         "",
         "### 条件选项（引擎动态生成）",
         "如果导演笔记（director_note）中包含 [条件选项] 行，说明引擎已根据玩家当前的属性/技能/物品/关系/环境",
         "扫描出了可行的特殊动作。你必须将这些选项**顺延编号**追加到标准选项之后（⑤ ⑥ ⑦ ⑧...）。",
         "引擎提供的格式为 `> ⑤ [条件]: 行动`，你可以用叙事语言润色「行动」部分的文字，但 `[条件]` 标签**必须保留**。",
         "引擎扫描规则：属性≥阈值 / 技能Lv / 持有物品 / NPC关系 / 身份声望 / 当前环境（家具/地点）。",
         "如果条件选项与当前叙事**严重脱节**（如场景已经离开教室，仍建议使用教室家具），可以跳过。"]),
  ].join("\n");
}
