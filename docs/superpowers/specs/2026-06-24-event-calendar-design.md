...
| `org_effects` | array | 组织级别的 NPC 行为覆盖。`override_action_template` 支持 `{role}` 和 `{role_action}` 变量，引擎从 NPC 的 tags/skills 推断填充（如 田径部→跑接力，学生会→组织开幕式） |
...
| | 引擎负责 | GM 负责 |
|---|---|---|
| 做什么 | 扫描日期范围内的事件，按 NPC 所属组织过滤 | 把事件文本写成角色特化的感知 |
| 产物 | 注入原始 `advance_hook` 作为素材 | 在 `spawn_npc_agent` 的 `sceneContext` 参数中覆写 |
| 例子 | 「体育祭 3 天后」 | 「你报了借物竞走，正在找人组队」 |
| 例子 | 「期末考 14 天后」 | 「你数学还完全没复习」vs「你已经在复习了」 |

引擎给的是素材，GM 写的是台词。引擎不编——同一个 `advance_hook` 文本原样注入给所有匹配 NPC 的 context，GM 负责按角色身份差异化。

NPC Agent 被 spawn 时，引擎自动执行：
1. 扫描当前日期 ± advance_days 范围内的事件
2. 按 NPC 所属组织过滤（P1 启发式 / P2 精确匹配）
3. 将匹配事件的 `advance_hook` 注入到 NPC 的 prompt 上下文

```
由比滨结衣 spawn 时引擎注入:
  [NPC·事件感知·素材]
    • 体育祭 3天后 — 体育祭临近，操场上各班级在练习接力
    • 期末考 14天后 — （暂无 advance_hook，仅日期提示）

GM 在 sceneContext 中覆写为:
  [NPC·事件感知]
    • 总武高体育祭 3天后 — 你报了借物竞走，正在找人组队
    • 期末考 14天后 — 你数学还完全没复习
```

如果 GM 不覆写，NPC 拿到的是引擎注入的原始 `advance_hook` 文本——能用但不特化。覆写是质量提升，不是必选项。
...
**现有 `region_contexts.json` 管的是氛围（这个地方感觉如何），不管事实（关于这个地方你知道什么）。** `data/orgs/` 补的是事实层——两层共存，不替换。详细边界见下方「和 region_contexts.json 的关系」节。
...
多条常识按触发匹配度排序，最多注入 5 条（避免膨胀）。排序规则：

1. **location 精确匹配** — 玩家所在具体房间/建筑名匹配（权重最高）
2. **location 层级匹配** — 面包屑路径中某层匹配（如 千叶_教育机构）
3. **orgs 匹配** — 玩家/NPC 所属组织名匹配
4. **topic 关键词匹配** — 对话中出现的关键词匹配
5. **roles 匹配** — 玩家/NPC 的身份标签匹配（权重最低）

同权重按条目在文件中的顺序。常去的地点（如 总武高/千叶市街）引擎可预缓存排序结果。5 条上限保留——常识是提示，不是百科。
...
#### 和 `region_contexts.json` 的关系

两个系统都按地点触发，但管的是不同维度。**不冲突——同地点触发两条独立的注入，进不同的 prompt 段。**

| | `region_contexts.json` | `data/orgs/` |
|---|---|---|
| 管什么 | 氛围——这个地方**感觉如何** | 事实——关于这个地方你**知道什么** |
| 注入位置 | `[区域设定]` 段 | `[常识]` 段 |
| 例子 | 「总武高走廊里阳光明媚，但空气中有微妙的竞争感」 | 「总武高偏差值68，千叶公立前5」 |
| 例子 | 「MAX咖啡是千叶灵魂饮料」 | 「千叶有四家咖啡连锁，MAX是本地品牌」 |
| 有 level 分级 | 无，进入地点即注入 | 有，common/industry/hidden |
| NPC 是否拿到 | 否，只有 GM | 是，NPC spawn 时按 level 过滤注入 |

进总武高时 GM 同时拿到两条独立的段：

```
[区域设定] 核心基调：Persona 5 风格的都市青春。光影平衡：白天明亮喧闹的日常…
[常识]
  • 总武高偏差值约68，千叶县公立校前5。
  • 校风自由，但校内严禁不纯异性交往。
```

两条来自不同文件，引擎分别匹配，合在一起才是完整的「在这个地方 GM 需要知道的东西」。
...
**大多数冲突 NPC 活不过一个场景。** 混混堵门、醉汉找茬、街头星探搭讪——这些角色用完即弃。塞进角色库是污染。下次 GM 再 spawn 一个混混，玩家不会问「这是上次那个吗」。

因此采用**先减后加**分层：

```
spawn_temp_npc（临时层）→ 场景结束自动回收
        ↓ 如果 GM 觉得有长期剧情价值
instantiate_npc（转正路径）→ 写入角色库，永久保留
```

`spawn_temp_npc` 是日常工具，`instantiate_npc` 是例外操作。GM 不需要在创建时判断「这个角色以后还会不会出现」——先创建，真有潜力再转正。
...
| | `create_character` | `spawn_temp_npc` | `instantiate_npc` |
|---|---|---|---|
| 用途 | 从零创建永久角色 | 即兴创建临时角色 | 模板路人 / 临时 NPC → 转正 |
| 输入 | 完整角色数据 | name + act + reason | 模板名 或 临时 NPC 名 |
| 持久化 | 永久在 DYNAMIC_CHARACTERS | 场景结束回收 | 永久写入角色库 |
| tk 成本 | ~3,000 | ~200 | ~300 |
| 互动能力 | 全部 | 对话/战斗/一次性记忆 | 全部 |
| 适用场景 | 新同学/新邻居/长期 NPC | 混混堵门/醉汉/街头偶遇 | 路人变重要 / 临时 NPC 有长期价值 |
...
如果 GM 在回收前认为某个临时 NPC 有长期剧情价值，调用 `instantiate_npc` 转正：

```
instantiate_npc({ temp_name: "混混A", reason: "有长期剧情价值" })
```

引擎行为：
1. 以临时 NPC 的已有数据（name/act/body_hint/hostility）为骨架，补全为完整 `StaticCharacter`
2. 写入 DYNAMIC_CHARACTERS，后续 `lookup_character` 永久可查
3. 从临时 NPC 列表中移除，跳过自动回收

GM 不需要提前判断——先 `spawn_temp_npc`，互动后觉得有潜力再转正。
...
第四阶段：临时 NPC（P4）
  ...
  设计：spawn_temp_npc（日常临时层）→ instantiate_npc（转正路径）
  改动：
    1. engine/types.ts — TempNPCState 类型
    2. engine/state.ts — 临时 NPC 存储 + 注入 + 回收逻辑
    3. tools/action/spawn_temp_npc.ts — 新工具
    4. tools/action/instantiate_npc.ts — 扩展支持 temp_name 参数转正
    5. tools/registry.ts — 注册
  依赖：无（可独立实现）
```
