...
**已完成 🎉**：
1. **两段式/三段式分层底座**：GM 结算轮、NPC 角色轮与渲染轮三阶段彻底物理隔离。
2. **秘密与揭示防火墙**：四级可见性与秘密解密日志就绪。
3. **叙事视角与双轨幕间系统**：自适应节奏检测（Novel/Turn-based）与单次长文本生成管道（短切镜/长幕间）完全落地，实现了心智隔离下的轻小说级叙事拉扯。

**当前应做的（本月黄金期）：**
1. **模块 C - NPC 记忆四层升级**：实现基于地点、人物、优先级的引擎级精准召回算法（`recallRelevantMemories`），并补全 Timeline 与 Tool 的写入链路。
2. **模块 D - 回合后 Review Agent**：在 `settle_scene` 同步阻塞引入 Flash 快速审计 LLM，智能拦截 OOC 与信息泄露，自动兜底关系变动。
3. **模块 E - state.ts 渐进式拆分**：将 4181 行的巨无霸 state.ts 按照星形无环依赖，星火式蚕食抽取为 `state-core.ts` 和多个子模块，杜绝内存 live-binding 赋值隐患。

**延后（P4 规划）的特性：**
- 手机端 Web 可视化前端（基于 Tavernlike API 对接）
- 战斗中队友指挥与扮演置换功能
- 赌博/买房/季节天气影响系统扩展
