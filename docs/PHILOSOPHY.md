...
│   Lint + retry（最多 3 次）                  │
├─────────────────────────────────────────────┤
│ pi agent echo: 原样输出预生成叙事正文        │
│   工具锁保护 → 即使带 tool defs 也没法调用    │
├─────────────────────────────────────────────┤
│ Phase 4: 创意层（可选，best-effort）          │
│   触发条件满足时调一次 LLM 做剧情判断         │
├─────────────────────────────────────────────┤
│ 下回合玩家输入 → before_agent_start          │
│   _toolsLocked = false → 解锁，重新开始      │
└─────────────────────────────────────────────┘
```
...
| Phase 3 | **pi agent loop**（有工具列表） | **裸 stream generateCompletion**（零工具结构隔离）。_toolsLocked=true 全程锁工具 · pi agent 仅负责 echo 预生成正文 |
...
| 日程 | NPC 按模板移动 + 覆盖 + 按年龄段分组 + 星期前缀区分 | GTA |
| 天气 | 马尔可夫链季节转移 + 温度模型 + 疲劳乘数 | — |
| 日历 | 日期触发 + 预热/当天/余波三阶段 + 组织效果 + 多日假期覆盖 | — |
...
| 怀孕 | 受孕计算（周期×避孕×幸运）+ 避孕药/堕胎 + 遗传生子 | — |
| 房地产 | 租赁/自有/欠款/存储 | — |
| 家具 | 放置/移除/交互/做容器 | 模拟人生 |
| 盗窃 | 引擎校验 + 失败自动触发关系惩罚 + 告警 flag | 博德之门 |
| 伪装 | identity_check → 暴露写入 flag.identity_exposed + 声望降 | — |
| 社交检定 | D20 × (3动作×3风格) × (性格/好感/幸运/死敌/恋人修正) | D&D |
| 全球大势 | tech/stability/tension 数值 + 分支条件 + 记忆染色 + 三段自然语言翻译 | — |
| 视角切换 | Player↔NPC 双向实体置换 + 关系网镜像 + Timeline beat 驱动 | — |
| 通勤偶遇 | 纯叙事注入，不建房间不移NPC。载具感知+社团分流+路线加成 | — |
...
| 在 engine/ 里硬编码角色名/地名 | 引擎零题材硬编码是核心铁律。所有世界数据在 worldpacks/（§八） |
| 建通勤房间实现偶遇 | 纯叙事注入比物理拦截更干净——不建房间、不移NPC、不需下车收口 |
| 手动写每角色×7天日程 | LLM 比引擎更懂"由比滨金曜会去哪"。NPC Agent 自主声明去向，引擎做兜底（§十二） |
...
| NPC Agent 统一 prompt 真相源 | `tools/state/spawn_npc_agent.ts` → `generateNpcAgentResponse()`（← autoSpawnNPCs, checkProactiveNPCs, spawn_npc_agent tool 三条路径） |

---

## 十二、NPC 自主日程：LLM 决定，引擎兜底

### 12.1 为什么不让引擎写每个人的日程

引擎写每人×每天×每时段的日程 = 数据地狱。17 个子系统已经够复杂了，不能再多一个 20角色×7天×3时段 的手写矩阵。

更根本的问题是：**引擎不知道由比滨金曜晚上为什么想去卡拉OK。** 这是角色内在驱动力——只有角色自己（通过 NPC Agent LLM）能合理判断。

### 12.2 LLM 自主 + 引擎兜底

NPC Agent prompt 注入今天的生活上下文（星期、天气、性格、最近记忆、社交关系）→ NPC 可选择在输出末尾附 `{"schedule_intent": {"location": "...", "action": "...", "reason": "..."}}` → 引擎解析后写入 `pendingOverride` → `updateNPCSchedules` 执行。
...
单日活动（体育祭→操场）和多日假期（暑假42天→全自由）是同一机制的不同尺度。`CalendarEntry.schedule_override` 直接覆写日程组模板，不需引擎知道"暑假"是什么概念——只需知道"这个日期窗口内这一组去哪"。

这也是"引擎零硬编码"的延伸：`schedule_override` 的 key 对应 `schedule_templates.json` 的组名，引擎不内置任何组名。

### 12.5 星期模板区分：引擎提供语境

引擎不应手动写"水曜下午去哪""金曜晚上去哪"——这是数据地狱。但引擎应该提供区分能力：`slotMap` 生成星期前缀 key（`水_afternoon`/`金_evening`），模板优先命中星期 key，不存在则回退通用的 `weekday_afternoon`/`weekday_evening`。

只在有明显不同于通用的天写 key——水曜短缩、金曜社交——其他天自然回退。引擎不内置"水曜=短缩"的规则，数据层决定。`dayKey` 是引擎动态拼接的（`星期_时段`），星期名来自 `gameState.time.day_of_week`。
