...
test("HUD: item-detail 使用消耗品真落地（回血+移除，不推正文）", async () => {
  ...
  hudEnterPanel(widget);
  // 焦点序：identity0 titles1 rep2 relations3 body4 equip5 skills6 bag0=7
  for (let i = 0; i < 7; i++) widget.handleInput("\x1b[B");
...
test("HUD: item-detail 装备落地 + 原槽物品回背包", async () => {
  ...
  hudEnterPanel(widget);
  for (let i = 0; i < 7; i++) widget.handleInput("\x1b[B"); // bag0
  widget.handleInput("\r");
  widget.render(80);
...
test("HUD: equip-detail 卸下真落地", async () => {
  ...
  hudEnterPanel(widget);
  for (let i = 0; i < 5; i++) widget.handleInput("\x1b[B"); // equip=5
...
test("HUD: 性征摘要行渲染 + 字段缺失兜底 + Enter 进 body-detail", async () => {
  ...
  // 焦点序：identity0 titles1 rep2 relations3 body4 sex5 sex6 → ↓×5 停在性状态行，Enter → body-detail
  for (let i = 0; i < 5; i++) widget.handleInput("\x1b[B");
  widget.handleInput("\r");
  outStr = widget.render(80).join("\n");
  if (!outStr.includes("SEX")) throw new Error("性状态行 Enter 应进 sex-detail");
...
test("HUD: sex-detail 双方面板（自身+相手两段）", async () => {
  ...
  hudEnterPanel(widget);
  // 焦点序：identity0 titles1 rep2 relations3 body4 sex5 sex6 → ↓×5 停在性状态行（sex index 0），Enter → sex-detail
  for (let i = 0; i < 5; i++) widget.handleInput("\x1b[B");
  widget.handleInput("\r");
