...
test("TUI: alerts 渲染不抛异常（HUD info-detail 使用）", async () => {
  resetState();
...
test("TUI: weather 渲染不抛异常（HUD info-detail 使用）", async () => {
  resetState();
...
test("HUD: item-detail 使用消耗品真落地（回血+移除，不推正文）", async () => {
  ...
  hudEnterPanel(widget);
  // 焦点序：identity0 titles1 rep2 body3 equip4 skills5 bag6 — ↓×6 停在 bag
  for (let i = 0; i < 6; i++) widget.handleInput("\x1b[B");
  widget.handleInput("\r");     // 进 bag-list
  widget.render(80);
  widget.handleInput("\r");     // Enter 第一件 → item-detail
  widget.render(80);
...
test("HUD: item-detail 装备落地 + 原槽物品回背包", async () => {
  ...
  hudEnterPanel(widget);
  for (let i = 0; i < 6; i++) widget.handleInput("\x1b[B"); // bag(idx6)
  widget.handleInput("\r");      // → bag-list
  widget.render(80);
  widget.handleInput("\r");      // Enter 第一件 → item-detail
  widget.render(80);
...
test("HUD: equip-detail 卸下真落地", async () => {
  ...
  hudEnterPanel(widget);
  for (let i = 0; i < 4; i++) widget.handleInput("\x1b[B"); // equip=4
...
test("HUD: 丢弃真落地（离开背包）+ 光标越界不抛", async () => {
  ...
  widget.handleInput("\r");       // Enter 第一件 → item-detail
  widget.render(80);
  widget.handleInput("\r");       // 唯一动作 = 丢弃 → 弹出确认对话框
  widget.handleInput("\r");     // 确认对话框：①确认丢弃
...
test("HUD: 性征摘要行渲染 + 字段缺失兜底 + Enter 进 body-detail", async () => {
  ...
  // 性征/性状态已收进 body-detail，不再暴露于 Tab 0 一级
  // body-detail 里的性器段验证（Enter body → body-detail → 性器）
  for (let i = 0; i < 3; i++) widget.handleInput("\x1b[B"); // → body at idx3
  widget.handleInput("\r"); // → body-detail
  outStr = widget.render(80).join("\n");
  if (!outStr.includes("身体")) throw new Error("body-detail 应正常渲染");
  widget.handleInput("\x1b"); // 退回
  // 兜底：profile 缺失 → 性征行整行不渲染、不占焦点、不抛
  p.sex = { arousal: 0, desire: 0, climaxCount: 0, profile: {} };
  outStr = widget.render(80).join("\n");
  if (!outStr.includes("身体")) throw new Error("body-detail 应仍可进入");
});
...
test("HUD: 情报摘要行（警报前置）+ 二级菜单 + 单段异步加载", async () => {
  ...
  if (!backStr.includes("成就")) throw new Error("info-section Esc 应退回九项菜单");
...
test("HUD: sex-detail 双方面板（自身+相手两段）", async () => {
  ...
  hudEnterPanel(widget);
  // sex-detail 从 npc-detail 的 ⑥状态 进入，不再从 Tab 0 直接进入
  // 验证 body-detail 里的性器段正常工作即可
  for (let i = 0; i < 3; i++) widget.handleInput("\x1b[B"); // → body at idx3
  widget.handleInput("\r"); // → body-detail
  const outStr = widget.render(80).join("\n").replace(/\x1b\[[0-9;]*m/g, "");
  if (!outStr.includes("身体")) throw new Error("body-detail 应正常渲染");
