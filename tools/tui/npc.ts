 ...
 */
import { showPanel, showMenu, pi } from "../helpers.ts";
...
async function showTalkMenu(name: string, done: () => void, ctx: any) {
  const items = [
    { label: "💬 聊聊日常", detail: "随意闲聊一些生活琐事", action: async () => { done(); await pi.sendUserMessage(ctx, `我找 ${name} 随便聊了聊日常琐事。`); } },
...
async function showTouchMenu(gameState: any, saveState: () => void, name: string, done: () => void, ctx: any) {
  ...
  const items: any[] = [
    { label: `🤝 友好握手 ${aff >= 10 ? "" : "(好感10+)"}`, detail: aff >= 10 ? "进行礼貌的肢体互动" : "关系还不够熟...", action: aff >= 10 ? async () => { done(); const r = touchResult(true, "友好握手", 2, 2); await pi.sendUserMessage(ctx, `我主动跟 ${name} 握了握手。${r}`); } : undefined },
    { label: `👋 亲切摸头 ${aff >= 30 ? "" : "(好感30+)"}`, detail: aff >= 30 ? "轻轻抚摸对方的头发" : "需要更多信任...", action: aff >= 30 ? async () => { done(); const ok = Math.random() > 0.15; const r = touchResult(ok, "亲切摸头", 2, 5); await pi.sendUserMessage(ctx, ok ? `我轻轻地伸手摸了摸 ${name} 的头，${name} 微微低下了头。${r}` : `我突然伸手想要摸 ${name} 的头，但${name}警惕地退后躲开了。${r}`); } : undefined },
    { label: `🤗 温暖拥抱 ${aff >= 50 ? "" : "(好感50+)"}`, detail: aff >= 50 ? "张开双臂给予拥抱" : "关系还不够亲密...", action: aff >= 50 ? async () => { done(); const ok = Math.random() > 0.2; const r = touchResult(ok, "温暖拥抱", 3, 10); await pi.sendUserMessage(ctx, ok ? `我走上前张开双臂，${name}犹豫了一下，轻轻靠了过来。${r}` : `我张开双臂想抱 ${name}，但${name}伸手挡住了我。${r}`); } : undefined },
  ];

  if (gameState.layer1Enabled) {
    items.push({ label: `💆 肢体按摩 ${aff >= 60 ? "" : "(好感60+)"}`, detail: aff >= 60 ? "为对方揉捏肩膀放松身体" : "需要更深的关系...", action: aff >= 60 ? async () => { done(); const ok = Math.random() > 0.25; const r = touchResult(ok, "肢体按摩", 3, 10); await pi.sendUserMessage(ctx, ok ? `我帮 ${name} 捏了捏肩膀，${name}的身体逐渐放松下来。${r}` : `我的手刚碰到 ${name} 的肩膀，${name}就躲开了。${r}`); } : undefined });
  }
...
export async function showNPCInteractionMenu(name: string, ctx: any) {
  ...
  const aff = getAffection(gameState, name);
  if (isInParty) {
    subItems.push({ label: "👥 移出队伍", detail: "将对方移出当前队伍", action: async (done: () => void) => { gameState.player.party = gameState.player.party.filter((n: string) => n !== name); saveState(); done(); await pi.sendUserMessage(ctx, `我把 ${name} 移出了队伍。`); } });
  } else {
    const canInvite = aff >= 40 || isLover(gameState, name);
    subItems.push({ label: `👥 邀请组队 ${canInvite ? "" : "(好感40+或恋人)"}`, detail: canInvite ? "邀请对方加入你的队伍" : "关系还不够铁...", action: canInvite ? async (done: () => void) => { gameState.player.party ??= []; gameState.player.party.push(name); saveState(); done(); await pi.sendUserMessage(ctx, `我邀请 ${name} 加入了我的队伍。${name}点了点头，跟了上来。`); } : undefined });
  }
  ...
  for (const [sName, sv] of Object.entries(gameState.player.skills || {}) as [string, any][]) {
    ...
    if (lvl <= 0) continue;
    if (sName === "医疗" || sName === "治疗") { subItems.push({ label: "🩹 医疗包扎", detail: `使用${sName}技能 (Lv.${lvl})`, action: async (done: () => void) => { done(); await pi.sendUserMessage(ctx, `我使用${sName}技能对 ${name} 进行伤势包扎治疗。`); } }); }
    else if (sName === "说服" || sName === "口才" || sName === "话术") { subItems.push({ label: "🗣️ 尝试劝说", detail: `使用${sName}技能 (Lv.${lvl})`, action: async (done: () => void) => { done(); await pi.sendUserMessage(ctx, `我施展${sName}技巧，试图说服 ${name}。`); } }); }
    else if (sName === "暗示" || sName === "催眠") { subItems.push({ label: "🌀 潜意识暗示", detail: `使用${sName}技能 (Lv.${lvl})`, action: async (done: () => void) => { done(); await pi.sendUserMessage(ctx, `我凝视着 ${name} 的眼睛，尝试施加${sName}。。。`); } }); }
  }
